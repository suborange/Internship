...
public class ChaseState : BaseAIState
{
    ...
    public override Type Tick()
    {
        ...
        transform.position = Vector3.MoveTowards(transform.position, _obj.Target.position, _step);
        ...
        var distance = Vector3.Distance(transform.position, _obj.Target.position);       
        // chase the player when close to eachother, then attack when in range
        if ( distance <= _range)
        {
            if (_obj._Melee == true && _obj.attack == true) return typeof(AttackState);
            if (_obj._Range == true && _obj.shoot == true) return typeof(ShootState);
        }
        // chase the player until player is too far away, then start to wander around.
        else if (distance > _range + 8f && _obj.wander == true)
        {
